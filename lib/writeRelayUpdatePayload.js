...
function _prepareSimpleRangeAdd(writer, payload, operation, config, isOptimisticUpdate) {
  ...
  require('fbjs/lib/invariant')(parentID, 'writeRelayUpdatePayload(): Cannot handle new list element without a configured ' + '`parentID` or a `%.id` field.', config.parentName);

  var nodeID = getString(newElement || {}, ID) || require('./generateClientID')();
...
function handleRangeAdd(writer, payload, operation, config, isOptimisticUpdate) {
  ...
  if (config.newElementName) {
    ...
    var _ret = function () {
      ...
      nodeID = getString(edgeNode, ID) || require('./generateClientID')();
...
function addRangeElement(writer, operation, config, parentID, newElementID, newElementData, isOptimisticUpdate) {
  var recordWriter = writer.getRecordWriter();
  var store = writer.getRecordStore();
  var rangeBehavior = require('./getRangeBehavior')(config.rangeBehaviors, []);
  ...
  var path = require('./RelayQueryPath').create(require('./RelayQuery').Root.build('writeRelayUpdatePayload', config.newElementName, null, null, {
    identifyingArgName: null,
    identifyingArgType: null,
    isAbstract: true,
    isDeferred: false,
  ...
  var nodeField = require('./RelayQuery').Field.build({
  ...
  path = require('./RelayQueryPath').getPath(path, nodeField, newElementID);
