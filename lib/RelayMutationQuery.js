...
var RelayMutationQuery = {
  ...
  buildFragmentForElementInsertion: function buildFragmentForElementInsertion(_ref4) {
    ...
    var rangeBehavior = require('./getRangeBehavior')(rangeBehaviors, []);
    /* eslint-disable no-console */
    if (process.env.NODE_ENV !== 'production' && console.groupCollapsed && console.groupEnd) {
    ...
    if (parentName != null) {
      ...
      if (trackedParent) {
        ...
        // );
        var mutatedParent = require('./intersectRelayQuery')(trackedParent, fatParent);
        if (mutatedParent) {
          mutatedFields.push(mutatedParent);
        }
      }
...
function buildElementField(parentID, listName, mutatedListFields) {
  ...
  var listField = require('./flattenRelayQuery')(require('./RelayQuery').Field.build({
    children: fields,
    fieldName: listName,
    metadata: { canHaveSubselections: true },
    type: ANY_TYPE
  }));
  require('fbjs/lib/invariant')(listField instanceof require('./RelayQuery').Field, 'RelayMutationQuery: Expected a field.');
  return listField;
}
