...
var RelayRecordWriter = function () {
  ...
  RelayRecordWriter.prototype._applyOptimisticRangeElementUpdate = function _applyOptimisticRangeElementUpdate(parentID, mutatedFieldName, nodeID, operation) {
    ...
    var fieldValue = require('./RelayRecord').create(nodeID);

    if (!parentRecord) {
      parentRecord = require('./RelayRecord').create(parentID);
  ...
  RelayRecordWriter.prototype._applyServerRangeElementUpdate = function _applyServerRangeElementUpdate(parentID, mutatedFieldName, nodeID, operation) {
    ...
    require('fbjs/lib/invariant')(list, 'RelayRecordWriter: Cannot apply `%s` update to non-existent list ' + '`%s` with parent of ID: %s.', operation, mutatedFieldName, parentID);
    if (operation === REMOVE) {
